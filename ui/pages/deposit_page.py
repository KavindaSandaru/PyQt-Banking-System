...
from database import deposit, get_balance
...
class DepositPage(QWidget):
    ...
    def build_header(self):
        ...
        title = QLabel("Deposit")
        title.setObjectName("PageTitle")

        subtitle = QLabel("Add money to your available balance")
    ...
    def build_form(self):
        ...
        summary.addWidget(icon_label("deposit", COLORS["green"], "#123d35", 66))
        ...
        submit = QPushButton("Deposit Money")
        submit.setObjectName("PrimaryButton")
        submit.clicked.connect(self.make_deposit)
    ...
    def make_deposit(self):
        ...
        if amount is None:
            QMessageBox.warning(self, "Invalid Amount", "Enter a valid deposit amount.")
            return

        if deposit(self.account_number, amount):
            self.amount_input.clear()
            self.refresh()
            if self.on_complete:
                self.on_complete()
            QMessageBox.information(self, "Deposit Complete", "Money deposited successfully.")
        else:
            QMessageBox.warning(self, "Deposit Failed", "The deposit could not be completed.")
